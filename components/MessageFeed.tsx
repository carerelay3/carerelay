...
export function MessageFeed({ messages, mode = "live" }: { messages: DemoMessage[]; mode?: "demo" | "live" }) {
  ...
  return (
    <div className="surface-panel min-w-0 space-y-4 p-4 sm:p-6">
      ...
      {sorted.map(msg => (
        <div key={msg.id} className="min-w-0 rounded-3xl border bg-white/70 p-4 shadow-sm" style={{ borderColor: "var(--border)" }}>
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="min-w-0 break-words font-semibold" style={{ color: "var(--text)" }}>{msg.sender || "Unknown sender"}</div>
                <div className="text-xs" style={{ color: "var(--text-subtle)" }}>{new Date(msg.createdAt).toLocaleString()}</div>
              </div>
              <p className="mt-2 break-words text-sm" style={{ color: "var(--text-muted)" }}>{msg.body}</p>
